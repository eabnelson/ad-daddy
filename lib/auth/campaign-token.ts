...
export class CampaignTokenService {
  ...
  readonly #spend = new Map<string, { usedMinor: number; idempotency: Map<string, number> }>();
  ...
  async verify(
    ...
    const key = await crypto.subtle.importKey("raw", this.#secret as BufferSource, { name: "HMAC", hash: "SHA-256" }, false, ["verify"]);
  ...
  revoke(tokenId: string): void { this.#revoked.add(tokenId); }
  ...
  async authorizeSpend(
    ...
    if (!Number.isSafeInteger(request.amountMinor) || request.amountMinor < 1 || !request.idempotencyKey || request.idempotencyKey.length > 256) throw new Error("Campaign token spend request is invalid");
    ...
    const state = this.#spend.get(claims.tokenId) ?? { usedMinor: 0, idempotency: new Map<string, number>() };
  ...
  private async sign(payload: string): Promise<string> {
    const key = await crypto.subtle.importKey("raw", this.#secret as BufferSource, { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
  ...
  private validateClaims(claims: CampaignTokenClaims, now: Date): void {
    if (!claims || typeof claims !== "object" || !Array.isArray(claims.scopes) || claims.scopes.some((scope) => !TOKEN_SCOPES.has(scope))) throw new Error("Invalid campaign token claims");
    if (Date.parse(claims.expiresAt) <= now.getTime()) throw new Error("Campaign token is expired");
    if (Date.parse(claims.issuedAt) > now.getTime()) throw new Error("Invalid campaign token issue time");
