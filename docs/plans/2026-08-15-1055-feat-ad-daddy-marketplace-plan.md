...
- R1. A landing-page instruction links over HTTPS from the Ad Daddy origin to a versioned agent-readable setup document that can guide either side through installation; any installed skill or CLI version is pinned, signed, and checksum-verified before device enrollment.
...
- **Distribution compromise:** the setup document stays on the HTTPS Ad Daddy origin, installers pin signed versioned artifacts and checksums, and enrollment aborts on any integrity mismatch.
