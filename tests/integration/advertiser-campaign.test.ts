...
import { CampaignService, MemoryBrandVerificationRepository, MemoryCampaignRepository } from "../../packages/cli/dist/commands/campaign.js";
...
const draft = {
  ...
  brand: { name: "Neon", verifiedDomain: "neon.tech", verificationId: "brand_neon" },
...
function createCampaigns(budgets = new CampaignBudgetService()) {
  ...
  const funding = { requireCreditedCampaignDeposit: async () => ({ depositId: "deposit_test" }), withCreditedCampaignDeposit: async <T>(_input: unknown, action: () => Promise<T>) => action() };
  return new CampaignService(new MemoryCampaignRepository(), budgets, brands, funding);
}
...
test("agent prepares campaign but verified funded human approval gates production and inventory", async () => {
  ...
  await campaigns.fund("campaign_1", approval(), now);
...
test("funding retries are idempotent and approvals reject malformed time bounds", async () => {
  ...
  await campaigns.fund("fund_retry", approved, now);
  await assert.doesNotReject(campaigns.fund("fund_retry", approved, now));
  await campaigns.prepare({ ...draft, campaignId: "bad_time" });
  await assert.rejects(campaigns.fund("bad_time", { ...approval(), approvedCampaignId: "bad_time", approvedAt: "not-a-date" }, now), /human approval/i);
});
...
test("mixed cash and credits inventory keeps credits when the receiver has no payout address", async () => {
  ...
  await campaigns.fund("mixed_campaign", approved, now);
...
test("funding-pending campaigns cannot bypass production approval through pause and resume", async () => {
  ...
  await campaigns.fund("resume_bypass", approved, now);
...
test("missing brand verifications and non-allowlisted destinations never prepare or expose inventory", async () => {
  const budgets = new CampaignBudgetService();
  const campaigns = createCampaigns(budgets);
  await assert.rejects(campaigns.prepare({ ...draft, campaignId: "campaign_unverified", brand: { ...draft.brand, verificationId: "missing" } }), /server-verified brand ownership/i);
