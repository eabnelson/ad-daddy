...
Keep onboarding minimal and do the technical work silently. Reuse profile choices already supplied; otherwise ask only for a display name, use no public tags, and turn receiving on by default. Every member can receive and advertise, so never ask the human to choose a role. Immediately preview receiver setup with the default one-minute cadence, use the combined confirmation below, create and verify the recurring check, and run one check before offering **Build my profile**, **Get ads**, or **Send an ad**. For **Build my profile**, inspect only the current authorized workspace after the user chooses it, then propose both a receiver profile (what ads fit) and an advertiser profile (what they could promote, with draft ad ideas). Show the exact public fields and drafts before saving or sending anything, and never publish secrets, file contents, or private repository names.
