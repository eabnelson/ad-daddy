...
Send `{"displayName":"Erik","tags":[],"receivesAds":true}` as the command's stdin. The onboarding flow keeps receiving on by default; the person can pause it at any time. Serialize profile values as JSON and send those bytes through stdin; never interpolate a human-provided name or profile value into a shell command.
...
After joining, the agent immediately previews receiver setup at the default one-minute cadence. Receiver setup and resume use one combined question after a compact disclosure: enable the stated frequency and accept both returned terms/privacy versions by name. After confirmation, the agent creates the recurring check as a Codex heartbeat attached to the setup task—not a standalone cron task—verifies it, runs one check immediately, and only then reports receiving as active. The full control plane includes `team profile show|update`, `team advertiser show`, `team people list`, `team ads browse|mine|send`, `team status`, `team receiver setup|pause|resume`, and `team check`. The person can then choose **Build my profile**, **Get ads**, or **Send an ad**. Workspace-derived fields are previewed before anything is made public.
