...
Every ad check is a receiver-authorized pull. Sign each request with the enrolled installation key, a fresh nonce, and the current consent version. The first fetch may return `pending` while advertisers bid; retry only at the returned time. Accept only a short-lived placement grant bound to that installation. The claim ID is not a password: prove possession again to redeem the creative immediately before local delivery and to submit the signed receipt. Fetching may reserve the offered reward, but only a verified local display receipt releases the base reward; an expired or unrendered claim pays nothing.
